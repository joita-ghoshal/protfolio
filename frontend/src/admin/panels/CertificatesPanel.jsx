      ...
      <div className="admin-card p-8 md:p-10">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-4">
            <div className="w-14 h-14 rounded-2xl bg-primary/10 flex items-center justify-center"><FiAward className="w-7 h-7 text-primary" /></div>
            <div><h2 className="text-2xl font-bold text-primary-text">Certificates</h2><p className="text-secondary-text text-sm">Manage your certificates and credentials</p></div>
          </div>
        ...
        {showForm && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="border border-border rounded-2xl p-6 mb-6 bg-secondary-bg/50">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              ...
              <div>
                <label className="block text-sm font-medium text-primary-text mb-1.5">Issuer</label>
                <input type="text" name="issuer" value={form.issuer} onChange={handleChange} className="input-field" />
              </div>
              <div>
                <label className="block text-sm font-medium text-primary-text mb-1.5">Issue Date</label>
                <input type="date" name="issue_date" value={form.issue_date} onChange={handleChange} className="input-field" />
              </div>
              <div>
                <label className="block text-sm font-medium text-primary-text mb-1.5">Credential URL</label>
        ...
        <div className="border-t border-border pt-6">
          {certificates.length === 0 && !showForm ? (
            <div className="text-center py-12">
              ...
              <p className="text-secondary-text text-lg font-medium">No certificates yet</p>
              <p className="text-secondary-text text-sm mt-1">Click "Add Certificate" to showcase your credentials</p>
            </div>
          ) : (
            <motion.div variants={container} initial="hidden" animate="show" className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {certificates.map((cert) => (
                <motion.div key={cert._id} variants={item} className="p-5 rounded-xl bg-secondary-bg flex items-start gap-4 group">
                  <div className="w-12 h-12 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
                    {cert.image ? <img src={cert.image} alt="" className="w-full h-full rounded-xl object-cover" /> : <FiAward className="w-5 h-5 text-primary" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-primary-text truncate">{cert.title}</h4>
                    <p className="text-sm text-secondary-text mt-0.5">{cert.issuer}</p>
                    {cert.issue_date && <p className="text-xs text-secondary-text mt-1">{new Date(cert.issue_date).toLocaleDateString()}</p>}
                    <div className="flex items-center gap-2 mt-3">
                      {cert.credential_url && <a href={cert.credential_url} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded-lg text-gray-400 hover:text-primary hover:bg-primary/5 transition-colors"><FiExternalLink className="w-3.5 h-3.5" /></a>}
                      {cert.pdf_url && <a href={cert.pdf_url} target="_blank" rel="noopener noreferrer" className="p-1.5 rounded-lg text-gray-400 hover:text-primary hover:bg-primary/5 transition-colors"><FiFileText className="w-3.5 h-3.5" /></a>}
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    <button onClick={() => openEdit(cert)} className="p-1.5 rounded-lg text-gray-400 hover:text-blue-500 hover:bg-blue-50 transition-colors"><FiEdit2 className="w-3.5 h-3.5" /></button>
                    <button onClick={() => handleDelete(cert._id)} className="p-1.5 rounded-lg text-gray-400 hover:text-red-500 hover:bg-red-50 transition-colors"><FiTrash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </motion.div>
              ))}
            </motion.div>
